 ...
 * Often used when dealing with large numbers, such as timestamps, large
 * counters, or data sizes in applications.
 *
 * @see uint64_t
 * @see https://en.cppreference.com/w/c/types/integer
 */
 ...
 * such as cryptographic algorithms or certain high-precision mathematical
 * operations.
 *
 ...
 *
 * This type represents a 128-bit unsigned integer. It is supported as an
 * extension in GCC and Clang for architectures that can handle 128-bit wide
 * integers. It can store values from 0 to 2^128 - 1.
 *
 ...
 *
 * @details
 * `usize` is defined as an alias for `size_t`, which is the standard type in C
 * for representing sizes and memory-related operations. The size of `usize` is
 * platform-dependent: on 32-bit systems, it is typically 32 bits, and on 64-bit
 * systems, it is typically 64 bits. It is commonly used for memory allocation,
 * array indexing, and representing sizes of objects in memory.
 *
 * `usize` is guaranteed to be large enough to hold any object size, making it
 * useful in functions like `sizeof()` and when performing operations like
 * dynamic memory allocation.
 *
 * @note The exact size of `usize` is dependent on the architecture of the
 * system.
 *
 * @see size_t
 ...
 * It is typically used for operations that involve memory addresses or
 * pointers.
 *
 * @attention This type is used for pointer arithmetic involving signed
 * operations. However, it is **not safe** for general pointer arithmetic, and
 * using it for pointer arithmetic may lead to undefined behavior.
 *
 * @see intptr_t
 * @see https://en.cppreference.com/w/c/types/integer
 */
 ...
 *
 * This type represents a 32-bit floating point number, commonly known as
 * `float`. It is used for representing real numbers with single-precision in
 * scientific and general-purpose calculations.
 *
 ...
 *
 * This type represents a 64-bit floating point number, commonly known as
 * `double`. It is used for representing real numbers with double-precision for
 ...
 *
 * This type represents a 128-bit floating point number, commonly known as `long
 * double`. It provides quadruple-precision floating point storage, useful for
 * calculations that require very high precision.
 *
 * @attention The size of `long double` is architecture-dependent, and may vary
 ...
 *
 * This type provides a signed integer that is at least 8 bits wide. The
 * compiler selects the most efficient size for the target platform, ensuring
 * optimal performance.
 *
 ...
 *
 * @attention Be cautious of implicit type conversions when mixing signed and
 * unsigned types.
 *
 * @see https://en.cppreference.com/w/c/types/integer
 */
 ...
 *
 * This type guarantees at least 16 bits but may be larger for performance
 * reasons. It is ideal when precise size is less critical than processing
 * speed.
 *
 * @attention Consider potential memory overhead due to platform-specific
 * optimizations.
 *
 * @see https://en.cppreference.com/w/c/types/integer
 */
 ...
 *
 * A signed integer type ensuring a minimum of 32 bits while potentially being
 * larger for performance reasons.
 *
 * @attention Ensure compatibility with libraries expecting fixed-width types
 * like `int32_t`.
 *
 * @see https://en.cppreference.com/w/c/types/integer
 */
 ...
 *
 * An unsigned integer of at least 32 bits, chosen for maximum speed by the
 * compiler. Use it for unsigned data that doesn't require exact size
 * constraints.
 *
 * @attention This type might occupy more memory than expected on some
 * platforms.
 *
 * @see https://en.cppreference.com/w/c/types/integer
 */
 ...
 *
 * This type represents the largest signed integer that can be supported by the
 * platform, offering the maximum possible range for signed integers.
 *
 * It is typically used for operations requiring the largest possible signed
 * integer values.
 *
 * @see intmax_t
 * @see https://en.cppreference.com/w/c/types/integer
 ...
 *
 * This type represents the largest unsigned integer that can be supported by
 * the platform, offering the maximum possible range for unsigned integers.
 *
 * It is used in situations that require working with the largest unsigned
 * integer values.
 *
 * @see uintmax_t
 * @see https://en.cppreference.com/w/c/types/integer
 ...
 *
 * This type represents a Unicode character, typically used for storing
 * characters from extended character sets (e.g., UTF-16, UTF-32).
 *
 ...
 *
 * This type represents a wide character type used specifically for input/output
 * operations. It is commonly used for wide-character string processing.
 *
 * @see wint_t
 * @see https://en.cppreference.com/w/c/types/char
 */
 ...
 *
 * @details
 * This macro redefines `__asm__` to be equivalent to the `asm` keyword, which
 * is used in C and C++ to embed assembly language instructions directly into
 * the program.
 *
 * @note
 * - Inline assembly allows programmers to write low-level code within a
 * higher-level C program.
 ...
 *
 * @details
 * This macro defines a `wstring` struct used for representing a wide character
 * string in memory. The struct contains:
 ...
 *
 * @details
 * This macro defines a `string` struct used for representing a standard
 * character string in memory. The struct contains:
 * - `cap`: The capacity of the string in bytes.
